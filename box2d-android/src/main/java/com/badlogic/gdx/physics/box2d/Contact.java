 ...
 * @author mzechner */
public class Contact implements fr.byob.game.box2d.dynamics.Contact {
	// @off
	/*JNI
#include <Box2D/Box2D.h>
	...
	public WorldManifold getWorldManifold () {
		final int numContactPoints = jniGetWorldManifold(addr, tmp);

		worldManifold.numContactPoints = numContactPoints;
		worldManifold.normal.set(tmp[0], tmp[1]);
		for (int i = 0; i < numContactPoints; i++) {
			final Vector point = worldManifold.points[i];
			point.x = tmp[2 + i * 2];
			point.y = tmp[2 + i * 2 + 1];
		}
	...
	public void getWorldManifoldToOut(final fr.byob.game.box2d.collision.WorldManifold worldManifold) {
		...
		((WorldManifold) worldManifold).numContactPoints = numContactPoints;
		worldManifold.normal.set(tmp[0], tmp[1]);
		for (int i = 0; i < numContactPoints; i++) {
			final Vector point = worldManifold.points[i];
			point.x = tmp[2 + i * 2];
	...
	public fr.byob.game.box2d.collision.Manifold getManifold () {
		final long manifoldAddr = jniGetManifold(addr);
