...
public class Fixture implements fr.byob.game.box2d.dynamics.Fixture {
	// @off
	/*JNI
#include <Box2D/Box2D.h>
	...
	protected Fixture (final Body body, final long addr) {
		this.body = body;
		this.addr = addr;
	}

	protected void reset (final Body body, final long addr) {
		this.body = body;
		this.addr = addr;
		this.shape = null;
		this.userData = null;
	...
	public Shape getShape () {
		if (shape == null) {
			final long shapeAddr = jniGetShape(addr);
			if (shapeAddr == 0) {
				throw new RuntimeException("Null shape address!");
			}
			final int type = Shape.jniGetType(shapeAddr);
