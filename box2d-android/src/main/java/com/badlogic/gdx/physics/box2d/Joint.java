...
public/* abstract */class Joint implements fr.byob.game.box2d.dynamics.joints.Joint {
	// @off
	/*JNI
#include <Box2D/Box2D.h>
	...
	private final Vector anchorA = new Vector();

	public Vector getAnchorA () {
	...
	private final Vector anchorB = new Vector();

	public Vector getAnchorB () {
	...
	private final Vector reactionForce = new Vector();
	...
	public void getReactionForce(final float inv_dt, final Vector argOut) {
		argOut.set(getReactionForce(inv_dt));
	}

	/** user data **/
	// TODO Try to use the CPP Joint>userData
