...
public class LongMap<V> {
	...
	public LongMap (final int initialCapacity, final float loadFactor) {
	...
	public V put (final long key, final V value) {
		...
		final int index1 = (int)(key & mask);
		final long key1 = keyTable[index1];
		if (key1 == key) {
		...
		final long key2 = keyTable[index2];
		if (key2 == key) {
		...
		final long key3 = keyTable[index3];
		if (key3 == key) {
	...
	public void putAll (final LongMap<V> map) {
	...
	static public class Values<V> extends MapIterator<V> implements Iterable<V>, Iterator<V> {
		...
		public Array<V> toArray () {
			final Array array = new Array(true, map.size);
			while (hasNext) {
				array.add(next());
			}
			return array;
	...
	static public class Keys extends MapIterator {
		...
		public long next () {
			final long key = nextIndex == INDEX_ZERO ? 0 : map.keyTable[nextIndex];
			currentIndex = nextIndex;
			findNextIndex();
			return key;
		}
